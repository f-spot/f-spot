...
public class CameraFileSelectionDialog
{
	...
	private void CreateInterface ()
	{
		...
		preview_list_store = new ListStore(typeof(string), typeof(string), typeof(Pixbuf), typeof(int));
		file_tree.Model = preview_list_store;
		
		// FIXME: We should get the home directory through gnome, but there doesn't seem to be
		//a binding for either of the right functions right now, g_get_home_dir() in glib is what
		//we need here.
		string home_directory = Environment.GetEnvironmentVariable ("HOME");
		copied_file_destination.Text = home_directory;
	...
	{
		ProgressDialog pdialog = new ProgressDialog("Downloading Previews", ProgressDialog.CancelButtonType.Cancel, 
							    camera.FileList.Count, camera_file_selection_dialog);
		
		int index = 0;
		foreach (GPhotoCameraFile file in camera.FileList) {
			cancelled = pdialog.Update("Downloading Preview of " + file.FileName);
	...
	private bool SaveFiles()
	{
		...
		ProgressDialog pdialog = new ProgressDialog ("Saving Files...", 
							     ProgressDialog.CancelButtonType.Cancel, 
							     selected_rows.Length, camera_file_selection_dialog);
		...
		if (copied_file_destination.Text.Length == 0) {
			MessageDialog md = new MessageDialog (camera_file_selection_dialog, 
							      ...
							      MessageType.Warning, 
							      ButtonsType.Ok, 
							      "A destination directory must be chosen.");
			md.Run ();
			md.Destroy ();
			return true;
		}
		...
		foreach (TreePath cur_row in selected_rows)
		{
			...
			cancelled = pdialog.Update("Saved File " + filename);
		}
	...
	{
		if (saved_files != null && import_files_checkbox.Active == true) {
	...
	{		
		FileSelection fs = new FileSelection ("Select save directory...");
	...
	{
		return (sep == System.IO.Path.DirectorySeparatorChar || sep == System.IO.Path.AltDirectorySeparatorChar || sep == System.IO.Path.VolumeSeparatorChar);
	}
